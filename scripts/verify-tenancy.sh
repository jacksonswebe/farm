...
# 2. A scoped read sees its own tenant's records.
#    Asserted by identity, not by row count: a count would break the moment
#    anyone adds a record, and this suite must fail only for real isolation
#    breaks, never for a changed fixture.
check "tenant A sees its own record" \
      "1" "$(scoped "$ORG_A" "SELECT count(*) FROM incidents WHERE reference='INC-2026-0001';" | num)"

# 3. ...and cannot see the other tenant's.
check "tenant A cannot read tenant B's records" \
      "0" "$(scoped "$ORG_A" "SELECT count(*) FROM incidents WHERE reference='INC-2026-9001';" | num)"
...
# 5. WITH CHECK blocks writes into another tenant, not just reads.
...
# 6. The audit trail is append-only for the application role.
